...
const FormulaInsertOrUpdate = (props) => {
    ...
    console.log("url that invoked FormulaInsertOrUpdate is " + url);
    if (url.search(":") > -1) {
    ...
    const encodeFormulaInput = () => {
        var formulaInputObj = {
            "formulaId": formulaData.formulaId, 
            "serviceType": formulaData.serviceType,
            "serviceImg": formulaData.serviceImg,
            "serviceDate": formulaData.serviceDate,
            "serviceCharge": formulaData.serviceCharge,
            "formula": formulaData.formula,
            "webUserId": formulaData.webUserId  
        };
        console.log("encodeFormulaInput on next line");
        console.log(formulaInputObj);
        // turn the user input object into JSON then run that through 
        // a URI encoder (needed for security on server side, prevents 
        // server from hacks). 
        //return encodeURIComponent(JSON.stringify(formulaInputObj));
        return encodeURI(JSON.stringify(formulaInputObj));
    };
    ...
    React.useEffect(
        ...
        () => {
            ...
            console.log("AJAX call for email list");
            ajax_alt("email/getAll",

                function (obj) { // obj holds formuula list from AJAX call
                    console.log("email/getAll Ajax success");
                    if (obj.dbError.length > 0) {  // db error trying to read role list
                        setErrorObj(setProp(errorObj, "webUserId", obj.dbError));
                    } else {

                        // role fields (from role/getAll): webUserId, userRoleType. 
                        // sort alphabetically by role type (not by id)
                        obj.emailList.sort(function (a, b) {
                        ...
                        // UserData.webUserId gets set whenever the user clicks on the role pick list, 
                        ...
                        // will work. 
                        setFormulaData(setProp(formulaData, "webUserId", obj.emailList[0].webUserId)); // FIX
                        console.log("set initial formula id for web_user to be "+obj.emailList[0].webUserId);

                        if (action === "update") { //this is update, not insert, get webUser by the id
                            console.log("Now getting webUser record " + id + " for the update");
                            ajax_alt("/formulaFile/getById?formulaId=" + id,
                                function (obj) {
                                    if (obj.errorMsg.length > 0) { // obj.errorMsg holds error, e.g., db error
                                        ...
                                        console.log("got the formula file record for update (on next line)");
                                        console.log(obj);
                                        setFormulaData(obj); // prepopulate user data since this is update.
                                    }
                                },
    ...
    React.useEffect(() => {
        ...
        if (submitCount > 0) { // dont make the AJAX call on initial render
            ...
            ajax_alt("formulaFile/" + action + "?jsonData=" + encodeFormulaInput(),
        ...
        <table className="insertArea">
            <tbody>
                <tr>
                    <td>Formula Id: </td>
                    <td>
                        <input value={formulaData.formulaId} disabled />
                    </td>
                    <td className="error">
                        {errorObj.formulaId}
                    </td>
                ...
                <tr>
                    <td>Service Type: </td>
                    <td>
                        <input value={formulaData.serviceType} onChange=
                            {e => setFormulaData(setProp(formulaData, "serviceType", e.target.value))}
                        />
                    </td>
                    <td className="error">
                ...
                <tr>
                    <td>Service Image: </td>
                    <td>
                        <input value={formulaData.userImage} onChange=
                            {e => setFormulaData(setProp(formulaData, "serviceImg", e.target.value))}
                        />
                    </td>
                    <td className="error">
                        {errorObj.serviceImage}
                    </td>
                </tr>
                <tr>
                    <td>Service Date: </td>
                    <td>
                        <input value={formulaData.serviceDate} onChange=
                            {e => setFormulaData(setProp(formulaData, "serviceDate", e.target.value))}
                        />
                    </td>
                    <td className="error">
                ...
                <tr>
                    ...
                    <td>
                        <input value={formulaData.serviceCharge} onChange=
                            {e => setFormulaData(setProp(formulaData, "serviceCharge", e.target.value))}
                        />
                    </td>
                    <td className="error">
                        {errorObj.serviceCharge}
                    </td>
                </tr>
                <tr>
                    <td>Formula: </td>
                    <td>
                        <input value={formulaData.formula} onChange=
                            {e => setFormulaData(setProp(formulaData, "formula", e.target.value))}
                        />
                    </td>
                    <td className="error">
                        {errorObj.formula}
                ...
                <tr>
                    ...
                    <td>
                        <select onChange=
                            {e => setFormulaData(setProp(formulaData, "webUserId", e.target.value))}
                            value={formulaData.webUserId}
                        >
                            {
                                emailList.map(email =>
